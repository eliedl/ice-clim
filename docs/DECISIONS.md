...
- **Validation status**: PENDING — awaiting (a) review of the first Minganie 100 m+1 km product (visual: coastal detail vs offshore smoothing, region clip, landmask), and (b) confirmation of `GRID_CRS=32198` for Minganie. The streaming-cube optimization and 25 m restoration are tracked as a follow-up under this DEC.
...
- **Context**: SGRDAGULF shapefiles carry **no CRS** in the shapefile/`.prj`; the `.xml` sidecar names the **WGS_1984 ellipsoid (spheroid)** but ingestion sees CRS-less geometry. The pipeline must assign a CRS before any downstream reprojection.
- **Options considered**:
...
- **Rationale**: The coordinates are geographic **lon/lat in degrees** (value ranges confirm), and the XML names the **WGS_1984 ellipsoid**. Geographic lon/lat on the WGS84 datum *is* EPSG:4326, so the relabel is correct. Crucially, `set_crs` attaches metadata **without moving any coordinate** — the right operation for a relabel; `to_crs` (which recomputes every vertex) would be wrong here. Nuance recorded for perennity: an ellipsoid is *necessary but not sufficient* to define a CRS (ellipsoid ⊂ datum ⊂ CRS); it is the combination *degrees + WGS84 ellipsoid/datum* that pins 4326, not the ellipsoid name alone. Residual risk: if the assumption were wrong the geometries would be silently misplaced (no error is raised) — corroborated, not proven, by the XML ellipsoid + degree-range coordinates. Contrast: SGRDAWIS28 *does* carry a CRS (polar stereographic) and takes the `to_crs(4326)` branch (pipeline.py:41-42).
- **Validation status**: PENDING — finalizing a domain assumption; corroborated by the XML `WGS_1984` ellipsoid + coordinate ranges; awaiting explicit sign-off.
- **Implementation refs**: `backend/ingestion/pipeline.py:39-42` (`set_crs` vs `to_crs` branches); `backend/ingestion/sources.py:35-40` (era CRS comments). Relates to DEC-031 (reprojection staleness of AREA/PERIMETER), DEC-039 (fetch-domain reprojection).
- **Literature cross-ref**: data/ingestion-chain decision; EPSG:4326 = WGS84 geographic 2D.

---

## DEC-039 — Chart Fetch Domain: Densified + Buffered Grid Envelope Reprojected to 4326

- **Context**: The analysis grid is built in the projected `grid_crs` (metres) from a region envelope (`build_grid`); the DB stores geometry in 4326. The SQL spatial filter must select **every** polygon the grid could rasterize. A straight envelope edge in `grid_crs` maps to a **curved** edge in 4326 (constant-Northing ≠ constant-latitude), so a filter built from the tight region geometry — or from a naive 4-corner reprojection — under-fetches the bowed grid-edge slivers.
- **Options considered**:
  1. **Filter with the tight region geometry in 4326** (status quo) — under-fetches the ~700 m bowed sliver at envelope edges.
  2. **Reproject only the 4 grid corners to 4326** — geopandas connects reprojected vertices with *straight chords*, which cut *inside* the true curve → same under-fetch.
  3. **Densify the grid envelope, buffer one cell, reproject to 4326** — the reprojected outline follows the true curve and the filter domain ⊇ grid domain.
- **Choice made**: Option 3 — `fetch_domain_wkt`: `box(envelope in grid_crs).segmentize(10·res).buffer(res).to_crs(4326).wkt` (pipeline.py:141-147).
- **Rationale**: Probe 010's second finding — the **2000-01-22** SE polygon (ymax 49.999°N < square ymin 50.0013°N) lived entirely in the bow sliver and was dropped from the median sample, producing a coherent **+7 d artifact over 1,155 cells**. `segmentize` adds dense vertices so the reprojected polyline approximates the true curve (reprojection draws straight chords between vertices; only dense sampling follows the bow); `buffer(res)` adds a one-cell margin so residual approximation error always errs toward **over-fetch**, which is harmless because rasterization assigns values only at in-grid cell centres. Regression: exact CIS/UQAR agreement 99.6 % → **99.8 %**, the SE difference component eliminated.
- **Validation status**: APPROVED (2026-06-11) — probe 010 attribution + post-fix regression.
- **Implementation refs**: `climatology/processing/pipeline.py` — `fetch_domain_wkt` (pipeline.py:123-147), `load_polygons` (pipeline.py:150-167); `backend/probes/010_cis_vs_uqar_freezeup_difference/README.md` § "Residual +7 d polygon"; `backend/probes/012_fetch_domain_reprojection_viz/` (visualization of the densify/buffer/reproject stack). Relates to DEC-036 (grid construction), DEC-027/DEC-035 (median-then-threshold sample integrity).
- **Literature cross-ref**: data/probe-chain decision (probe 010).
...
- **Rationale**: Recommended by the LDGIZC geographer as the deliverable CRS. Zone-independence gives one seamless frame for the whole province; conformality preserves shape; scale distortion is negligible relative to source resolution. **Resolution-floor framing**: CIS SIGRID-3 polygon resolution is the binding lower bound on product resolution because it exceeds the projection's ground impact — *were CIS resolution infinite, the projection scale error would become the lower bound*. `res_m` is projection-plane metres; ground discrepancy ≤ ~0.2 m at 100 m, negligible vs charting precision.
- **Validation status**: PENDING — awaiting (a) **probe 013** (scale-factor / distortion map quantifying max |k−1| per region at 25/100/1000 m), (b) decision to migrate legacy regions (a recompute, not a relabel), and (c) geographer/user sign-off.
- **Implementation refs**: `climatology/processing/pipeline.py` `GRID_CRS` (26919 → 32198 proposed); `climatology/processing/regions.py` `ADAPTIVE_GRID_CRS=32198` (already); `climatology/utils/square_bbox.py` `WORK_CRS`; `backend/probes/013_qc_lambert_scale_factor/` (distortion map). Relates to DEC-036 (adaptive grid, already 32198), DEC-031 (reprojection staleness).
- **Literature cross-ref**: LDGIZC geographer recommendation; EPSG:32198 (NAD83 / Québec Lambert, LCC 2SP, SPs 46°N/60°N).
