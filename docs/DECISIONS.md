...
- **Implementation refs**: `climatology/services/units_conversion_maps.py` — `TRACE_CONCENTRATION = 0.03` (defined before `CONCENTRATION_FRACTION`, which references it for `01`/`02`); `_sd_concentration` rewritten to the `CT_eff` rule; `RESIDUAL_TRACE_EPSILON` removed; `attribute_polygon` computes `ct_eff`. `climatology/tests/test_attribution.py` (docstrings updated; assertions unchanged, all pass). Probe re-run: `backend/probes/001_sd_residual/probe.py` (+ output `2026-07-01_*.txt`) — adds the `CT_eff`/bucketing equivalence section.
