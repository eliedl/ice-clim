...
| 015 | Coarse-tier grid lattice shift — quantifies the bbox origin / cell-size offset from the DEC-041 bounds trim that drives its ~99 % (not 100 %) reproducibility | **complete** 2026-06-17 — feeds DEC-041. Manicouagan origin shift fractional 0.758 (x) / 0.555 (y) cell, cell-size −0.41/−2.12 ‰ → the ±7 d one-cell isochron fringe (98.93 % exact). See `015_coarse_bounds_lattice_shift/README.md`. |
| 018 | Form-of-ice code census (`FA/FB/FC`, SGRDA + SGRDR) — data-driven domain for a `FORM_SIZES` map, form-axis analog of probe 002 | **complete** 2026-07-01 — valid set `{01..08, 10}` + `99`/`-9`; fast ice `08` dominates `FA` (sgrda 257k / sgrdr 130k), rare in `FB`/`FC`; `{2C,5C,9C}` encoding errors (18 rows, sgrda only). `FORM_SIZES` values NEEDS REVIEW. See `018_form_of_ice_census/README.md`. |
| 019 | Landfast (fast ice, form `08`) concentration characteristics — inform a fast-ice climatology kernel in `metrics.py` | **complete** 2026-07-01 — landfast ⟺ `CT=1.0` both ways (`FA='08'`→`CT='92'` 99.9%; `CT='92'`→`FA='08'` 99.3% sgrda / 99.0% sgrdr; remainder = compact drift floes). Reuse existing `EventDate`/`ThresholdCount` at threshold 1.0. Per-polygon ~1% contamination is an upper bound; median-level error to be measured (proxy-vs-direct diff). See `019_landfast_concentration/README.md`. |
| 020 | Landfast CT=1.0 proxy validation — median-level cell diff of the proxy (CT≥1.0) vs a direct `FA='08'` indicator climatology (sept-îles 2011–2020) | **complete** 2026-07-01 — **proxy validated**: freeze-up 100.00% exact, duration 99.15%, breakup 97.13%, all Δ≤2 d, **0 presence mismatch**. Median-then-threshold fully suppresses the per-polygon ~1% compact-drift contamination. See `020_landfast_proxy_validation/README.md`. |
| 021 | CT=1.0 proxy vs published CIS 1991–2020 EC fast-ice freeze-up normals (`fifup.shp`) — external validation, probe-010 pattern | **complete** 2026-07-01 — **externally validated**: 76 738 common cells, diff median +0.0 d / std 0.8 d (no bias), 99.1% within ±half-week, value range identical (Jan 01–Feb 26). CT=1.0 proxy reproduces the CIS fast-ice freeze-up normals. See `021_landfast_vs_cis_fifup/README.md`. |