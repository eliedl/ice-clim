...
# CIS Historical Date calendar (DEC-027, e116): 52 fixed month-days per year.
# Weekly except the Nov 26 -> Dec 4 8-day jump; the leap-year exception is the
# Feb 26 -> Mar 5 interval (8 days in leap years, 7 otherwise) — the month-days
# themselves never change. Empirically confirmed by the 2026-06-10 full-record
# sgrdr/ec run: exactly these 52 month-days carry coverage ratio ~0.9.
# Note: CIS climatological products for the East Coast (EC) region carry no
# Sep/Oct HDs; they are retained here so any older-era charts in those months
# still bin to an HD and surface in the cross-era coverage comparison.
...
HD_LABELS = [f"{m:02d}-{d:02d}" for m, d in HD_MONTH_DAYS]
